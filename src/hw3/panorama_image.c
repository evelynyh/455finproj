...
match *match_descriptors(descriptor *a, int an, descriptor *b, int bn, int *mn)
{
    ...
    *mn = an;
    ...
    for(j = 0; j < an; j++){
        ...
        float dist = 0;
        float best = l1_distance(a[j].data,b[bn-1].data, b[bn-1].n);
        for(i = 0; i < bn; i++){
            dist = l1_distance(a[j].data, b[i].data, b[i].n);
            if(best > dist) {
                bind = i;
                best = dist;
            }
...
float point_distance(point p, point q)
{
    ...
    //return sqrtf(((p.x*p.x)-(p.y*p.y)))+sqrtf(((q.x*q.x)-(q.y*q.y)));
}
...
image cylindrical_project(image im, float f)
{
    ...
    image c = make_image(im.w + zxp + zxp, im.h, im.c);
    ...
    for (int k = 0; k < im.c; k++) {
        for (int x = 0; x < im.w; x++) {
            for (int y = 0; y < im.h; y++) {
                ...
                set_pixel(c, x + zxp, y, k, bilinear_interpolate(im, xp-.5, yp-.5, k));
            }
        }
    }
    return c;
